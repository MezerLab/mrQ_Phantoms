function T1FitExperimentData_phantom(loadStr, saveStr, method, checkData)
%
...
%%origanly fitStr   = ['[T1Est bMagEst aMagEst res] = T1Fit' method '(data(jj,:),nlsS);'];
if strcmp(method,'NLSPR')
method=strcat(method,'_phantom')
end
fitStr   = ['[T1Est,T1EstTmp, bMagEst,bEstTmp, aMagEst,aEstTmp, res,resTmp] = T1Fit' method '(data(jj,:),nlsS);'];
...
mesures=size(data,4);
maskFactor = 0.003;  %%origanly 0.03
...
for ii=1:nSteps
    ...
    for jj = curInd:endInd
     ...
     'll_T1(jj,:) = [T1Est bMagEst aMagEst res];';
       ...
       T1_FTI(jj,:,ti)=  [T1EstTmp(ti) bEstTmp(ti) aEstTmp(ti) resTmp(ti)];
        end
...
for ii=1:nbrOfFitParams
 for ti=1:size(data,2)
    ...
    im2(maskInds)= T1_FTI(:,ii,ti);
    T1_InveMaps(:,:,:,ii,ti)=im2;
...
if (savefitdata)
    ...
    save(strcat(saveStr,'_Multi_Maps'),'T1_InveMaps')
end
