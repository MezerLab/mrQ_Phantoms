function [M01, BM1,boxSize, skip , Segmask,meanVal, XX, YY, ZZ]= ...
    mrQ_GetPhantomM0_boxData(opt,M0,BM,fb,smoothkernel,seg,Inclusion_Criteria)
%[M01, t1, BM1,boxSize, skip , Segmask,meanVal, XX, YY, ZZ]= ...
%    mrQ_GetM0_boxData(opt,T1,M0,BM,fb,smoothkernel,seg,Inclusion_Criteria)
%
% Loads  data from the M0 and T1 file for a box defined by fb and opt
%
%  ~INPUTS~
%                   opt: The structure of optimization parameters
%                    M0:
%                    BM:
%                    fb:
%          smoothkernel:
%                   seg:
%    Inclusion_Criteria: Default is [0.8 200].
%
%   ~OUTPUTS~
%                   M01:
%                   BM1:
%               boxSize: The 4D size of the box
%                  skip:
%               Segmask:
%               meanVal:
%                    XX:
%                    YY:
%                    ZZ:
%
% Helper routine to get the data


M01=[];  BM1=[];boxSize=[]; meanVal=[];skip=0;Segmask=[];

%smoothe (default is no)
if ~notDefined('smoothkernel') 
    if smoothkernel>0;
    for ii=1:size(M0,4)
        tmp=M0(:,:,:,ii);
        M0(:,:,:,ii)=smooth3(tmp,'g',smoothkernel);
    end
    
    
    end
end

if notDefined('Inclusion_Criteria') 
Inclusion_Criteria=[0.4 200];
end

XX(1)=opt.X(fb(1),fb(2),fb(3))-opt.HboxS(1);
XX(2)=opt.X(fb(1),fb(2),fb(3))+opt.HboxS(1);
YY(1)=opt.Y(fb(1),fb(2),fb(3))-opt.HboxS(2);
YY(2)=opt.Y(fb(1),fb(2),fb(3))+opt.HboxS(2);
ZZ(1)=opt.Z(fb(1),fb(2),fb(3))-opt.HboxS(3);
ZZ(2)=opt.Z(fb(1),fb(2),fb(3))+opt.HboxS(3);
    
    %get the location of the box we will work on (in image space (x,y,z))

% Pull out the data
M01 = M0(XX(1):XX(2),YY(1):YY(2),ZZ(1):ZZ(2),:);

% This is the 4D size  of the box
boxSize = size(M01);

% Sorting the M0 data according to the mean value of the data.
% SNR, biggest to smallest
[meanVal, coilIndex] = sort(squeeze(mean(mean(mean(M01)))),'descend');
M01     = M01(:,:,:,coilIndex);
meanVal = meanVal(coilIndex);
  
BM1=logical(BM(XX(1):XX(2),YY(1):YY(2),ZZ(1):ZZ(2)));
    
if ~notDefined('seg') 
seg=seg(XX(1):XX(2),YY(1):YY(2),ZZ(1):ZZ(2));
Segmask=zeros(size(seg));
 Segmask(find(seg==1))=1; %We only use Agar mask that are mark by one.
%    TissueType= unique(seg)';
%   k=1;
%    for ii=TissueType
%        if  length(find(seg==ii))>20 % if there are almost no voxels for a tisue type, we won't use it for regularization
%            if ii~=0;
%            Segmask(find(seg==ii))=k;
%            k=k+1;
%            end
%        end
%    end
   if  length(find(Segmask))<(length(Segmask(:))*0.2) ||  length(find(Segmask))<Inclusion_Criteria(2)% we need R1 data for regularization. If we don't have it, we won't use this box
       skip=1;
   end
   
end   
   


% if length(find(BM1))<length(BM1(:))*Inclusion_Criteria(1) ||  length(find(BM1))<Inclusion_Criteria(2)  % not enghf voxels
%     skip=1;
% end
%     

