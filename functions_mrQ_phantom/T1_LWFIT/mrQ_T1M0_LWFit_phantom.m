function [T1_WL, T1_L,PD_WL, PD_L] = mrQ_T1M0_LWFit_phantom(s,brainMask,tr,flipAngles,Gain,B1,outDir,xform,mrQ_struc,GridOutputDir,savenow)
%
...
if isfield(mrQ_struc.spgr.lwfit,'SunGrid')
    SGE=mrQ_struc.spgr.lwfit.SunGrid;
else
    SGE=0;
end

if isfield(mrQ_struc.spgr.lwfit,'sub')
    sb=mrQ_struc.spgr.lwfit.sub;
else
    sb='UN';
end
 sb=[sb '_3dT1PD'];
...
id_str=(mrQ_struc.spgr.lwfit.sub);
name_save_pwd=fullfile('~/',id_str);

mkdir(name_save_pwd);
name_save_pwd=fullfile(name_save_pwd,'opt_lw');
save(name_save_pwd,'opt');

mrQ_struc.spgr.lwfit.LWoptname=logname;
save( mrQ_struc.name,'mrQ_struc');
%% III. Perform the optimization (optional: use the SunGrid)
%% III-a. USE THE SGE
id=str2num(mrQ_struc.spgr.lwfit.sub);
clear brainMask tmp Res M0 options
if SGE==1;
    mkdir (fullfile(pwd,'/cluster_loglw'))
    ...
    if (~exist([outDir '/tmpSGWL'],'dir')), mkdir([outDir '/tmpSGWL']);
        ...
        for jobindex=1:ceil(length(opt.wh)/jumpindex)
            ...
            command=sprintf('qsub -cwd -j y -b y -N job_%g -o cluster_loglw "matlab -nodisplay -r ''mrQ_fitT1PDLW_SGE_phantom(%f,%g,%g); exit'' >cluster_loglw/log"', jobname,id,jumpindex,jobindex);
        ...
        %         if proclass==1
        %             sgerun2('mrQ_fitT1PDLW_SGE_phantom(opt,8000,jobindex);',sgename,1,1:ceil(length(opt.wh)/jumpindex));
        %         else
        %             sgerun('mrQ_fitT1PDLW_SGE_phantom(opt,8000,jobindex);',sgename,1,1:ceil(length(opt.wh)/jumpindex));
        %         end
  ...
  if has_PCTbox == 0 %no PCTbox, and thus no parfor
      if ~isempty(MissingFileNumber)
          for kk=1:length(MissingFileNumber) %regular for-loop
              jobindex=MissingFileNumber(kk);
              mrQ_fitT1PDLW_SGE_phantom(id,jumpindex,jobindex);
          end
      end
