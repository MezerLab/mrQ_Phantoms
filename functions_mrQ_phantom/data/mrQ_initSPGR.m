function [s,xform,mmPerVox,niiFiles,flipAngles,mrQ_struc] = ...
    mrQ_initSPGR_phantom(spgrDir,refImg,mmPerVox,interp,skip,clobber,mrQ_struc)
%
% [s,xform,mmPerVox] = ...
...
if notDefined('interp')
    interp = 1;
    mrQ_struc.spgr.init_interp=interp;
end
...
if exist('mrQ_struc','var') && ~isempty(mrQ_struc) && isfield(mrQ_struc,'autoacpc')
    autoAcpc = mrQ_struc.spgr.autoacpc;
else
    autoAcpc = 0;
end
...
if isfield(mrQ_struc.spgr,'inputdata') %infut of list of nifti file and the relevant scan parameters
    [s niiFiles]=mrQ_input2Stuck(mrQ_struc.spgr.inputdata,0);
    t1Inds(1:length(s))=1;
    t1Inds=logical(t1Inds);

elseif  isfield(mrQ_struc.spgr,'raw_strac')
    load(mrQ_struc.spgr.raw_strac);
     t1Inds(1:length(s))=1;
    t1Inds=logical(t1Inds);
else
...
if process
    ...
    if  isfield(mrQ_struc.spgr,'raw_strac')
    else
    for ii = 1:numel(niiFiles)
        [s1(ii), mrQ_struc.spgr.coilNum(ii)]= makeStructFromNifti(niiFiles{ii},-2,s(ii),mrQ_struc.spgr.permution);
    end
    clear s
    s = s1;
    clear s1
...
if ~isfield(mrQ_struc.spgr,'cheack')
    mrQ_struc.spgr.cheack=0;
end
...
if mrQ_struc.spgr.cheack==1
    ...
    mrQ_struc.spgr.Scan_Skiped_Num=find(t1Inds==0);
    end
...
mrQ_struc.spgr.niiFile=niiFiles;
flipAngles=[s(t1Inds).flipAngle];
mrQ_struc.spgr.niiFile_FA=flipAngles;
...
if ~exist('mmPerVox','var') || isempty(mmPerVox),
    ...
    mrQ_struc.spgr.init_mmPerVox=mmPerVox;
...
if process
    ...
    if ~exist('refImg','var') || isempty(refImg)
        ...
        refImg = t1w_acpcfile;
        mrQ_struc.spgr.init_ref_acpc=refImg;
...
% make nii from the images
    ...
    mrQ_sruc.spgr.initDir=outDir;
    mrQ_struc.spgr.outDir=outDir;
